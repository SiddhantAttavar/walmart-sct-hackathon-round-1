...
float haversine(float lat1, float lon1, float lat2, float lon2) {
	...
	return round(res * 100.0f) / 100.0f;
}
...
float tsp(int i, int m, std::vector<std::vector<float>> &memo, std::vector<std::vector<std::pair<int, int>>> &par, std::vector<std::vector<float>> &dist) {
	...
	if (memo[i][m] != std::numeric_limits<float>::infinity()) {
		return memo[i][m];
	}
...
int main() {
	int num_nodes;
	std::cin >> num_nodes;
	std::vector<std::pair<float, float>> locs(num_nodes);
	locs[0] = {-1, -1};
	for (int i = 0; i < num_nodes; i++) {
		std::cin >> locs[i].first >> locs[i].second;
	}

	std::vector<std::vector<float>> dist(num_nodes, std::vector<float>(num_nodes, 0));
	for (int i = 0; i < num_nodes; i++) {
	...
	std::vector<std::vector<float>> memo(num_nodes, std::vector<float>(1 << num_nodes, std::numeric_limits<float>::infinity()));
	std::vector<std::vector<std::pair<int, int>>> par(num_nodes, std::vector<std::pair<int, int>>(1 << num_nodes, {-1, -1}));
